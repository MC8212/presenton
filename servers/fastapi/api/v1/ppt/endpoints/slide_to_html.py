...
class SlideToHtmlRequest(BaseModel):
    ...
    provider: Optional[str] = None  # LLM provider: "openai", "google", "anthropic", "custom"
...
async def generate_html_from_slide(
    ...
    """
    Generate HTML content from slide image and XML using the specified LLM provider.
    ...
    Args:
        ...
        provider: LLM provider to use (openai, google, anthropic, custom)
    ...
    print(f"Generating HTML from slide image using {provider_name} provider...")

    # Build user text with OXML and fonts
    fonts_text = (
        f"\nFONTS (Normalized root families used in this slide, use where it is required): {', '.join(fonts)}"
        if fonts
        else ""
    )
    user_text = f"OXML: {xml_content}\n{fonts_text}"

    # Use VisionLLMService for multi-provider support
    vision_service = VisionLLMService(provider=provider)
    html_content = await vision_service.generate_with_vision(
        system_prompt=GENERATE_HTML_SYSTEM_PROMPT,
        user_text=user_text,
        images=[{"base64": base64_image, "media_type": media_type}],
        max_tokens=8000,
    )
...
async def generate_react_component_from_html(
    ...
    """
    Convert HTML content to TSX React component using the specified LLM provider.
    ...
    """
    provider_name = provider or "default"
    print(f"Generating React component using {provider_name} provider...")

    user_text = f"HTML INPUT:\n{html_content}"

    # Build images list (optional)
    images = []
    if image_base64 and media_type:
        images.append({"base64": image_base64, "media_type": media_type})

    # Use VisionLLMService for multi-provider support
    vision_service = VisionLLMService(provider=provider)
    react_content = await vision_service.generate_with_vision(
...
async def edit_html_with_images(
    ...
    """
    Edit HTML content based on one or two images and a text prompt using the specified LLM provider.
    ...
    """
    provider_name = provider or "default"
    print(f"Editing HTML using {provider_name} provider...")

    user_text = f"CURRENT HTML TO EDIT:\n{html_content}\n\nTEXT PROMPT FOR CHANGES:\n{prompt}"

    # Build images list
    images = [{"base64": current_ui_base64, "media_type": media_type}]
    if sketch_base64:
        images.append({"base64": sketch_base64, "media_type": media_type})

    # Use VisionLLMService for multi-provider support
    vision_service = VisionLLMService(provider=provider)
    edited_html = await vision_service.generate_with_vision(
...
async def convert_slide_to_html(request: SlideToHtmlRequest):
    """
    Convert a slide image and its OXML data to HTML using the specified LLM provider.

    Args:
        request: JSON request containing image path, XML content, and optional provider
    ...
    try:
        ...
        # Generate HTML using the VisionLLMService
...
async def convert_html_to_react(request: HtmlToReactRequest):
    ...
    Args:
        request: JSON request containing HTML content and optional provider
    ...
    try:
        ...
        # Convert HTML to React component using VisionLLMService
...
async def edit_html_with_images_endpoint(
    ...
    """
    Edit HTML content based on one or two uploaded images and a text prompt using the specified LLM provider.
    ...
    try:
        ...
        # Edit HTML using VisionLLMService
